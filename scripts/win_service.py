...
from analyze_traffic import analyze_traffic_patterns, load_and_clean_data, generate_html_report
from cicflowmeter_utils import run_cicflowmeter

# 生产环境配置
SCAN_INTERVAL = 3600  # 扫描间隔（秒）- 1小时
TARGET_IPS = ["192.168.1.1", "8.8.8.8", "127.0.0.1"]  # 监控目标IP列表
            ...
            while self.is_running:
                ...
                for ip in TARGET_IPS:
                    ...
                    try:
                        ...
                        if "error" in anomaly_result:
                            ...
                            servicemanager.LogInfoMsg(f"IP {ip} 异常检测完成：发现{len(anomaly_result['anomaly_results'])}个结果")
                    except Exception as e:
                ...
                try:
                    ...
                    if "error" in analysis_report:
                        ...
                        # 生成HTML报告
                        html_path, msg = generate_html_report(analysis_report)
                        if html_path:
                            servicemanager.LogInfoMsg(f"HTML报告已生成：{html_path}")
                        else:
                            servicemanager.LogWarningMsg(f"HTML报告生成失败：{msg}")
                        ...
                        if self.es_client:
                            try:
                                ...
                                if not df.empty:
                                    ...
                                    servicemanager.LogWarningMsg("无数据插入ES：数据为空")
                            except Exception as e:
